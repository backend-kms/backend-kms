<h3>👋 About Me</h3>

<p>
Backend Developer with 4 years of experience building production backend systems
with Django, Django REST Framework, and FastAPI.
</p>

<p>
I focus on designing reliable APIs, authentication workflows, and maintainable
service architectures that scale with real business requirements.
</p>

<p>
I plan and develop APIs based on service requirements, while also working on
third-party integrations, internal system interoperability, performance improvements,
and refactoring legacy code into clearer, testable structures.
</p>

<p>
With an M.S. in Artificial Intelligence, I'm also interested in applying AI to
developer productivity, workflow automation, and practical backend engineering problems.
</p>

<p>
I value simple code, clear ownership, code reviews, and documentation that help teams
move faster without sacrificing stability.
</p>

<blockquote>
  <strong>KR.</strong>
  서비스 요구사항을 바탕으로 API를 기획하고 개발하며, 인증 흐름, 외부 서비스 연동,
  성능 개선, 레거시 리팩토링 등 실제 서비스 운영에 필요한 백엔드 문제를 해결해 왔습니다.
</blockquote>

<br/>

<h3>🚀 What I Work On</h3>

<ul>
  <li>Designing and maintaining production REST APIs</li>
  <li>Implementing authentication, authorization, and user flow logic</li>
  <li>Integrating third-party APIs and connecting internal systems</li>
  <li>Optimizing backend performance and database access patterns</li>
  <li>Refactoring business logic into clearer, testable code</li>
</ul>

<br/>

<h3>🧰 Tech Stack</h3>

<h4>Languages</h4>
<p>
  <img src="https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white"/>
  <img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black"/>
  <img src="https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white"/>
</p>

<h4>Backend</h4>
<p>
  <img src="https://img.shields.io/badge/Django-092E20?style=flat-square&logo=django&logoColor=white"/>
  <img src="https://img.shields.io/badge/Django%20REST-092E20?style=flat-square&logo=django&logoColor=white"/>
  <img src="https://img.shields.io/badge/FastAPI-009688?style=flat-square&logo=fastapi&logoColor=white"/>
  <img src="https://img.shields.io/badge/NestJS-E0234E?style=flat-square&logo=nestjs&logoColor=white"/>
</p>

<h4>Data Layer</h4>
<p>
  <img src="https://img.shields.io/badge/PostgreSQL-4169E1?style=flat-square&logo=postgresql&logoColor=white"/>
  <img src="https://img.shields.io/badge/MySQL-4479A1?style=flat-square&logo=mysql&logoColor=white"/>
  <img src="https://img.shields.io/badge/SQLite-003B57?style=flat-square&logo=sqlite&logoColor=white"/>
  <img src="https://img.shields.io/badge/MongoDB-47A248?style=flat-square&logo=mongodb&logoColor=white"/>
  <img src="https://img.shields.io/badge/Django%20ORM-092E20?style=flat-square&logo=django&logoColor=white"/>
  <img src="https://img.shields.io/badge/SQLAlchemy-D71F00?style=flat-square&logo=sqlalchemy&logoColor=white"/>
  <img src="https://img.shields.io/badge/TypeORM-F98B2B?style=flat-square&logo=typeorm&logoColor=white"/>
  <img src="https://img.shields.io/badge/ODMantic-47A248?style=flat-square&logo=mongodb&logoColor=white"/>
</p>

<h4>API / Realtime / Cache</h4>
<p>
  <img src="https://img.shields.io/badge/Swagger%20%2F%20OpenAPI-85EA2D?style=flat-square&logo=swagger&logoColor=black"/>
  <img src="https://img.shields.io/badge/Redis-DC382D?style=flat-square&logo=redis&logoColor=white"/>
  <img src="https://img.shields.io/badge/WebSocket-010101?style=flat-square&logo=socketdotio&logoColor=white"/>
</p>

<h4>Infrastructure &amp; Server</h4>
<p>
  <img src="https://img.shields.io/badge/AWS-232F3E?style=flat-square&logo=amazonwebservices&logoColor=white"/>
  <img src="https://img.shields.io/badge/EC2-FF9900?style=flat-square&logo=amazonec2&logoColor=white"/>
  <img src="https://img.shields.io/badge/S3-569A31?style=flat-square&logo=amazons3&logoColor=white"/>
  <img src="https://img.shields.io/badge/Route%2053-8C4FFF?style=flat-square&logo=amazonroute53&logoColor=white"/>
  <img src="https://img.shields.io/badge/ELB-8C4FFF?style=flat-square&logo=awselasticloadbalancing&logoColor=white"/>
  <img src="https://img.shields.io/badge/Lightsail-232F3E?style=flat-square&logo=amazonlightsail&logoColor=white"/>
  <img src="https://img.shields.io/badge/Nginx-009639?style=flat-square&logo=nginx&logoColor=white"/>
  <img src="https://img.shields.io/badge/Gunicorn-499848?style=flat-square&logo=gunicorn&logoColor=white"/>
  <img src="https://img.shields.io/badge/uWSGI-2F2F2F?style=flat-square&logo=uwsgi&logoColor=white"/>
</p>

<h4>Workflow &amp; Tools</h4>
<p>
  <img src="https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white"/>
  <img src="https://img.shields.io/badge/GitHub%20Actions-2088FF?style=flat-square&logo=githubactions&logoColor=white"/>
  <img src="https://img.shields.io/badge/GitHub%20Copilot-000000?style=flat-square&logo=githubcopilot&logoColor=white"/>
  <img src="https://img.shields.io/badge/Figma-F24E1E?style=flat-square&logo=figma&logoColor=white"/>
  <img src="https://img.shields.io/badge/Slack-4A154B?style=flat-square&logo=slack&logoColor=white"/>
  <img src="https://img.shields.io/badge/Notion-000000?style=flat-square&logo=notion&logoColor=white"/>
  <img src="https://img.shields.io/badge/GA4-E37400?style=flat-square&logo=googleanalytics&logoColor=white"/>
  <img src="https://img.shields.io/badge/GTM-246FDB?style=flat-square&logo=googletagmanager&logoColor=white"/>
</p>
